...
C_M0_w = (C_m0_airf_0 * ((AR_w * cosd(LAMBDA_14_w) * cosd(LAMBDA_14_w)) / (AR_w + 2 * cosd(LAMBDA_14_w))) - 0.01 * eps1_w) * (a_w / a_w_0) ;
...
for i_v = [-3:5]
    for alf = [-10:10]
        ...
        C_M_cg(alf+11) = (-C_L_w * (x_w - x_cg) / cbar) + C_M0_w + (C_Mf * alf) - (eta_v * C_L_v * (S_v / S_w) * ((x_v - x_cg) / cbar) + ((Z_t * T) / (q * S_w * cbar))) ;
